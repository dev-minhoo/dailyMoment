...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	 ...
	 * .authorizeRequests() .antMatchers("/user/**").permitAll() // LoadBalancer Chk
	 * .anyRequest().authenticated() .and() .formLogin() .loginPage("/user/login")
	 * // .loginProcessingUrl("/user/loginAuth") .usernameParameter("id")
	 * .passwordParameter("pwd") .defaultSuccessUrl("/family/1", true) .permitAll()
	 * .and() .logout(); }
	 * 
	 ...
	 */
	    ...
	    protected void configure(HttpSecurity http) throws Exception {
	        ...
	        http.httpBasic().disable()
	                ...
	                .antMatchers("/user/**").permitAll() // 회원 로그인 및 회원가입
